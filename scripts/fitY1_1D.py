...
plt.title(args.catver+' blinded '+tp+' '+zr+r' $\alpha=$'+str(alpha)+'$\pm$'+str(err))
plt.grid()
plt.legend()
...
ax.errorbar(mod[0],xid[indmin:indmin+nbin]-mod[2],diag[indmin:indmin+nbin],fmt='o',color=color)
...
#plt.ylabel(r'$\xi_0-\xi_{0,{\rm mod}}$')
#plt.title(args.catver+' blinded '+tp+' '+zr)
plt.title(' blinded '+tp[:3]+' '+str(args.zmin)+'<z<'+str(args.zmax),size=16)
ax.set_box_aspect(1)
ax.set_yticklabels([])
